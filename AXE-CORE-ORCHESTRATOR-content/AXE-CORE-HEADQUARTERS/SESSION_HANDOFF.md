...
## Nightly conversation self-review — now genuinely server-side too (this session, with VPS access)
No longer only client-side. `run_conversation_review()` was ported to Python in `backend/axe_api/main.py`, scored via local Ollama (`hermes3:8b` on the VPS, not the user's cloud provider keys — those only ever live in the app), and hooked into `/cron/tick` (same trigger as memory decay's pg_cron job, but this one can't be pg_cron itself since Postgres can't call an LLM directly without a Vault-held key + `pg_net` round-trip — a straight VPS-side call to the Ollama that's already running locally was the simpler, equally-"always-on" path). The client-side `maybeNightlyReview()` in `axeBootstrap.ts` was left in place deliberately — harmless double coverage on a day the app happens to be open, and the day-scoped `core_background_job_runs` claim means only one of the two actually does the LLM work.

**Real bug this surfaced and fixed**: two uvicorn workers both saw the review as "due" on ticks 9s apart (pass takes ~90s, cron ticks every 60s) and both ran it — duplicate `core_conversation_reviews` rows for the same exchanges (cleaned up manually). New `core_background_job_runs` table (migration `20260728_core_background_job_runs.sql`) gives an atomic claim via its primary key, not just a read-then-write marker check.

Verified live: manually fired `/cron/tick` 3x concurrently, all three correctly saw today's review as already claimed and skipped — no duplicate run.
