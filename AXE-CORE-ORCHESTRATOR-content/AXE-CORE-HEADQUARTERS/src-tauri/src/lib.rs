...
use tauri::Manager;
// Menu, tray en venster-events bestaan alleen op desktop (achter de tray-icon-
// feature). Op Android zijn die modules er niet.
#[cfg(desktop)]
use tauri::{
    menu::{Menu, MenuItem},
...
/// Welke machine deze installatie is. Geen extra crate: `hostname` bestaat op
/// desktop; op Android mag hij ontbreken, dan blijft alleen os/arch over.
#[tauri::command]
...
/// Op Android bestaan die diensten niet: de telefoon is client, geen host.
#[tauri::command]
...
fn dienst_start(id: String) -> Result<String, String> {
    ...
    {
        let _ = id;
        Err("Lokale diensten bestaan alleen op de desktop-app.".into())
    }
}
...
/// Desktop-instappunt én Android-entry. De binary (main.rs) en de mobile
/// library delen dezelfde run(), zodat het desktopgedrag niet een tweede
/// kopie wordt.
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Tray en venster-verbergen bestaan alleen op desktop; op mobiel
            // is er geen tray en sluit je de app niet weg naar een balk.
            #[cfg(desktop)]
            {
