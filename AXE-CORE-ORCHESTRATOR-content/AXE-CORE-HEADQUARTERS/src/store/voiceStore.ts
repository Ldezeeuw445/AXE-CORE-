...
import { logMessage } from '@/services/memory/coreDB';
import { classifyQueryDynamic, loadCapabilities, getAgentSystemPrompt, getCapabilityExecutionMode } from '@/services/ai/capabilityService';
import { buildWorkflow, formatBuildResult } from '@/services/ai/workflowBuilder';
import { getSystemSummary, checkAllServices } from '@/services/platform/systemService';
import { getDefaultOllamaModelNames, sortOllamaModelsForCapability } from '@/services/ai/ollamaModelCatalog';
import { getStoredLlmModelRegistry } from '@/services/ai/llmModelRegistryService';
...
import { loadMessages, saveMessage, AXE_USER_ID, loadAllConversations, createNewConversationId, APP_SOURCE, saveConversationLocal, loadConversationLocal } from '@/services/memory/chatPersistence';
import type { ConversationSummary } from '@/services/memory/chatPersistence';
import { isAxeApiConfigured, crewRun, tts, checkAxeApi, apiExecuteOpenHands, apiExecuteOpenJarvis, apiExecuteOpenClaw, apiExecuteKiloCode, apiExecuteHermes } from '@/services/integrations/axeCoreApiService';
import { speakWithElevenLabs, stopTTS } from '@/services/integrations/elevenLabsService';
import { detectChatAction, type ChatAction } from '@/services/ai/chatActionService';
import { getEveSystemPromptSupplement } from '@/lib/eveSkills';
import { saveGlobalMemory, buildGlobalMemoryContext } from '@/services/memory/globalMemoryService';
import { getSupabase } from '@/core/supabase/client';
import { tavilySearch, tavilyConfigured, formatTavilyResults } from '@/services/integrations/tavilyService';
import { browseFetch, formatBrowseResult } from '@/services/integrations/browserFetchService';
  ...
  return{
    ...
    startListening:async()=>{
      try{
        ...
        if(googleSlot?.key){
          try{
            const{setGeminiLiveApiKey,getGeminiLiveService,startGeminiLive}=await import('@/services/integrations/geminiLiveService');
    ...
    sendMessage:async(text:string)=>{
      ...
      if(/\b(verander|wijzig|pas\s+aan|change|modify|update|fix|rename)\b/i.test(lower)&&/\b(tab|pagina|page|component|button|knop|kleur|color|stijl|style|tekst|text|header|menu|modal|sidebar|card|sectie|section)\b/i.test(lower)){
        const{isGitHubConfigured,findFile,readFile,writeFile}=await import('@/services/integrations/githubCodeService');
      ...
      try{
        const{classifyBranch}=await import('@/services/ai/langGraphOrchestrator');
        ...
        const{orchestrate}=await import('@/services/ai/langGraphOrchestrator');
