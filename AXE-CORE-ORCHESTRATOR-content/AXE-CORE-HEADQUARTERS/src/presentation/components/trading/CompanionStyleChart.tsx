/** Companion desk — uses CompanionChartDesk (Canvas + volumetric OB + Fib). */
...
import { getDemoAccount, executeDemoTrade, equity, unrealizedPnl, markPositions } from '@/infrastructure/persistence/demoTradingService';
...
export type IndicatorSnapshot = {
  ...
  fib382: number | null; fib618: number | null; bars: number;
};
...
function toBars(candles: { time: string; open: number; high: number; low: number; close: number }[]): Bar[] {
...
export function CompanionStyleChart({ symbol = 'XAUUSD', timeframe: tfProp = '1h', onIndicators, className }: Props) {
  ...
  const ohlc = useMemo(() => bars.map(b => ({ t: b.time * 1000, o: b.open, h: b.high, l: b.low, c: b.close })), [bars]);
  ...
  const candles = useMemo(() => barsToMetaApiCandles(bars.map(b => ({ time: b.time, open: b.open, high: b.high, low: b.low, close: b.close }))), [bars]);
  ...
  const snap: IndicatorSnapshot = useMemo(() => {
    ...
    return {
      ...
      fib382: f382?.price ?? null, fib618: f618?.price ?? null, bars: bars.length,
    };
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const mapped: Bar[] = snapM.bars.map(b => ({ time: Math.floor(b.t / 1000), open: b.o, high: b.h, low: b.l, close: b.c }));
        if (mapped.length) { setBars(mapped); setStatus(`${symbol} · ${tf} · public · ${mapped.length} bars`); }
        else setStatus('No candles — set MetaAPI token/account or use a public symbol');
      } catch (e) {
  ...
  return (
    <div className={className} style={{ display: 'flex', flexDirection: 'column', gap: 8, minHeight: 0, height: '100%' }}>
      ...
      <div className="flex items-center gap-2 flex-wrap rounded-xl px-3 py-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.28)' }}>Companion layer · volumetric OB · draggable Fib</span>
      </div>
    </div>
  );
}
