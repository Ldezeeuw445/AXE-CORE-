...
import { getAwarenessSnapshot, awarenessPrompt } from '@/application/awareness/axeAwareness';
...
export const useVoiceStore=create<VoiceState>((set,get)=>{
  ...
  return{
    ...
    sendMessage:async(text:string)=>{
      ...
      const awareness=await getAwarenessSnapshot();
      const messages=[{role:'system'as const,content:systemContent+awarenessPrompt(awareness)},...history.slice(0,-1),{role:'user'as const,content:text}];
