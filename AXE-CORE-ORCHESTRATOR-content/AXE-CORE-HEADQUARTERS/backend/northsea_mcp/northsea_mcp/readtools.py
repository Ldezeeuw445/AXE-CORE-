    ...
    async def dashboard_snapshot(self, caller: Caller, *, view: str | None = None) -> dict:
        ...
        views: dict[str, dict] = {
            ...
            "counterparties": {"backend": "northsea.py TAB_SQL['tegenpartijen']", "metrics": [
                ...
                _metric("do_not_contact", sum(1 for c in comp if canon.do_not_contact_reason(c)), "Integrity rule (canon.py), not a database column.",
            ...
            "communications": {"backend": "northsea.py TAB_SQL['communicatie'] (newest 300)", "metrics": [
                _metric("messages", len(comms), "All rows in communications.", ["communications"]),
                ...
                _metric("messages_last_7d", sum(1 for m in comms if ts(m.get("occurred_at")) >= _now_minus(7).timestamp()),
                        "occurred_at in the last 7 days (desk counter 'communicatie_7d').", ["communications"])]},
